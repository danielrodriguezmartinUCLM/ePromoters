...
#Importamos los scripts bed e interval
import bed
import interval

"""Función características de forma"""
class shapeCharacteristics():
	...
	def parseCharacteristics(self, line):
		fields = line.rstrip().split("\t")	#.split(divide una cadena en una lista)
											#.rstrip(elimina el caracter final de la cadena
...
"""Calcular números negativos"""
def calculateNumberNegatives(nonrandomFile, histone, opPrefix):
	"""
	La función de cálculo de números negativos calcula el número de regiones mo aleatorias con señal
	Argumentos:
		nonrandomFile: archivos no aleatorios (tipo de datos: string)
		histone: es el archivo de señal de histona (tipo de datos:string)
	Returns:
		numberNegativeIntervals número de posibles intervalos negativos posibles
	"""
...
"""Encontrar máximos circundantes"""
...
"""Calcular las características de los picos"""
...
"""Obtener regiones de doble pico"""
def getDoublePeakRegions(MPRApeaks, signal, metaMaxima1, metaMaxima2, metaMinima, op, op2, metaIntersectingIntervals, doublePeakRegions, otherMarkFile=""):
	"""
	Esta función encuentra las regiones de picos dobles alrededor de los picos de MPRA.
	Args:
		MPRApeaks: MPRApeaks (tipo de datos:list of Intervals)
		signal: es el archivo de señal de histonas (de tipo de datos:bigWig)
		metaMaxima1: es la lista con índices de todos los máximos en regiones de doble pico
		metaMaxima2: es la lista con todos los índices de segundos máximos en regiones de doble pico
		metaMinima: es la lista con todos los índices de mínimos en regiones de doble pico
		op: es un archivo en el que se escriben las características de doble pico
		op2: es un archivo de BED en el que se escriben intervalos de picos dobles
		metaIntersectingIntervals: es una lista de señales dentro de regiones de doble pico
	Returns:
		None
	"""
	width = 2000
...
"""Obtener intervalo correspondiente"""
...
def main(histoneFile, MPRApeakFile, nonrandomFile, opPrefix, pp, otherMarkFile=None, plotChar=True):
	...
	Calcula el patrón de valles entre picos de STARR-seq.
	Args:
		histoneFile: histoneFile: bigWigfile de señal de histona
		MPRApeaks: contiene picos de un ensayo informador masivamente paralelo para regiones reguladoras
		nonrandomFile: contiene regiones en las que los negativos no deben cruzarse
		opPrefix: prefijo de salida para todos los archivos de salida
	Returns:
